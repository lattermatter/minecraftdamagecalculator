...
mob = input("Enter mob attacked (Enter Q for options): ")
counter = 0
if mob == 'Q' or mob == 'q':
    print('Playable: ')
    for i in mob_hp:
...
armor_toughness = int(input("Enter the mob's armor toughness points: "))
...
# WEAPON
weapon = input('Enter weapon used (e.g. Iron Sword, Golden Axe, etc.): ')
weapon_type = ''
weapon_material = ''
index = 0
...
for letter in weapon:
    ...
    if letter == ' ':
        break
    weapon_material += letter
for letter in weapon[index:]:
    weapon_type += letter
if weapon != 'Trident':
...
print(
    f'''To get a(n) {mob} ({hp} HP) with a(n) {weapon} {crit_Bool},
...
The total damage done would be {round(total_dmg - hp, 2)} more than the mob's HP ({hp}).
''')
